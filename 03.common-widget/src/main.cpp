...
int main(int argc, char *argv[])
{
    ...
    QGuiApplication::setApplicationName(QStringLiteral("Qt5 03 Common Widget"));
    QGuiApplication::setApplicationVersion(QStringLiteral("0.1.0"));

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("Qt Quick 常用控件教学示例"));
    parser.addHelpOption();
    parser.addVersionOption();

    const QCommandLineOption smokeTestOption(
        QStringLiteral("smoke-test"),
        QStringLiteral("加载控件示例后自动退出。"));
